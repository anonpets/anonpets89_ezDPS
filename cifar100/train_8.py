...
test_label = []
ratio = 0.04
shuf = np.loadtxt('shuffledata_8.txt', dtype=int)
for i in shuf:
    print(i)
...
# t0 = time()
pca = pickle.load(open('pca_8.sav', 'rb'))
...
clf = pickle.load(open('pipeline_8.sav', 'rb'))
# #############################################################################
...
# correct_classified = []
# for i in range(len(test_sub_label)):
#     if test_sub_label[i] == y_pred[i]:
#         correct_classified.append(i)
# np.savetxt('shuffledata_8.txt', correct_classified, fmt='%i')
